...
def following(request):
    ...
    following_users_ids = []
    if request.user.is_authenticated:
        following_users_ids = Follow.objects.filter(follower=request.user).values_list('followed', flat=True)
    ...
    if request.method == 'POST':
        ...
        # Get only the quotes from the selected user
        quotes = Quote.objects.filter(user=selected_user).order_by('-timestamp')
    else:
